...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const latestTransationPromise = Order.find()
            .select(["total", "orderItems", "discount", "status"])
            .limit(4);
        ...
        const modifyLatestTransation = latestTransation.map((i) => ({
            _id: i._id,
            discount: i.discount,
            amount: i.total,
        ...
        stats = {
            ...
            categoryCount,
            userRatio,
            latestTransation: modifyLatestTransation,
        };
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const productCategories = await getCategoriesInventory({
            categories,
            productsCount,
        });
